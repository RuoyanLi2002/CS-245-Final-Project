...
"""
    ...
    def _analyze_context(self, task_description):
        ...
        try:
            ...
            user_reviews = self.interaction_tool.get_reviews(user_id=task_description['user_id'])
            item_reviews = self.interaction_tool.get_reviews(item_id=task_description['item_id'])
    ...
    def _validate_and_optimize(self, plan, context, task_description):
        ...
        has_user_step = any(
            any(keyword in step.get('description', '').lower() for keyword in required_keywords['user'])
            for step in plan
        )
        has_item_step = any(
            any(keyword in step.get('description', '').lower() for keyword in required_keywords['item'])
            for step in plan
        )
        has_reviews_step = any(
            any(keyword in step.get('description', '').lower() for keyword in required_keywords['reviews'])
            for step in plan
        )
        ...
        if not has_item_step:
            plan.insert(1 if has_user_step else 0, {
        ...
        for step in plan:
            if 'tool use instruction' in step:
                ...
                if isinstance(tool_inst, dict):
                    # 确保包含必要的 ID
                    if 'user_id' not in tool_inst and 'user' in step.get('description', '').lower():
                        tool_inst['user_id'] = task_description['user_id']
                    if 'item_id' not in tool_inst and ('item' in step.get('description', '').lower() or 'business' in step.get('description', '').lower()):
...
class ReasoningCOTVoting(ReasoningBase):
    ...
    def __init__(self, profile_type_prompt, llm, memory=None, num_candidates=5, num_votes=5):
        """
    ...
    def _extract_stars_and_review(self, text):
        ...
        try:
            ...
            if not stars_line or not review_line:
                ...
                review_match = re.search(r'review[:\s]+(.+?)(?=\n\n|\nstars?|$)', text, re.IGNORECASE | re.DOTALL)
...
class MySimulationAgent(SimulationAgent):
    ...
    def __init__(self, llm: LLMBase):
        ...
        self.planning = PlanningSmart(llm=self.llm, interaction_tool=None, memory=self.memory)
    ...
    def workflow(self):
        ...
        try:
            ...
            for sub_task in plan:
                ...
                tool_instruction = sub_task.get('tool use instruction', {}) or sub_task.get('tool_use_instruction', {})
                
                # Handle user information step
                if 'user' in description or 'profile' in description:
                    if isinstance(tool_instruction, dict) and 'user_id' in tool_instruction:
                ...
                elif 'business' in description or 'item' in description or 'product' in description:
                    if isinstance(tool_instruction, dict) and 'item_id' in tool_instruction:
            ...
            print(f"review_similar: {review_similar}")
            # . They are ranked from closest to this user's past review. Please combine these information when making a decision and prioritize the first past review
            task_description = f'''
            ...
            Others have reviewed this business before. Please consider following their sentiment and way of talking: {review_similar}
...
if __name__ == "__main__":
    ...
    simulator = Simulator(data_dir="/local2/liruoyan/245/AgentSocietyChallenge/datasets", device="gpu", cache=False)
    ...
    simulator.set_llm(OpenAILLM(api_key="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"))
    
    # Run the simulation
    # If you don't set the number of tasks, the simulator will run all tasks.
    outputs = simulator.run_simulation(number_of_tasks=1, enable_threading=True, max_workers=10)
    
    # Evaluate the agent
    evaluation_results = simulator.evaluate()       
    with open(f'./K10_{task_set}.json', 'w') as f:
